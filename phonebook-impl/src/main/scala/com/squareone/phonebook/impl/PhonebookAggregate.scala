...
/**
  * This provides an event sourced behavior. It has a state, [[PhonebookState]], which
  ...
  */
object PhonebookBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[PhonebookCommand]): Behavior[PhonebookCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, PhonebookEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[PhonebookCommand, PhonebookEvent, PhonebookState](
        persistenceId = persistenceId,
        emptyState = PhonebookState.initial,
...
/**
  ...
  */
case class PhonebookState(message: String, timestamp: String) {
  def applyCommand(cmd: PhonebookCommand): ReplyEffect[PhonebookEvent, PhonebookState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: PhonebookEvent): PhonebookState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[PhonebookEvent, PhonebookState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[PhonebookEvent, PhonebookState] =
    Effect
...
object PhonebookState {
  ...
  /**
    ...
    */
  def initial: PhonebookState = PhonebookState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[PhonebookCommand]("PhonebookAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[PhonebookState] = Json.format
}

/**
  * This interface defines all the events that the PhonebookAggregate supports.
  */
sealed trait PhonebookEvent extends AggregateEvent[PhonebookEvent] {
  def aggregateTag: AggregateEventTag[PhonebookEvent] = PhonebookEvent.Tag
}

object PhonebookEvent {
  val Tag: AggregateEventTag[PhonebookEvent] = AggregateEventTag[PhonebookEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends PhonebookEvent
...
/**
  ...
  */
trait PhonebookCommandSerializable

/**
  * This interface defines all the commands that the PhonebookAggregate supports.
  */
sealed trait PhonebookCommand
    extends PhonebookCommandSerializable
...
/**
  ...
  * application loader.
  */
object PhonebookSerializerRegistry extends JsonSerializerRegistry {
